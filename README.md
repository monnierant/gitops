...
spec:
  source:
    ...
    targetRevision: HEAD
    plugin:
      name: argocd-vault-plugin   # ou avpall, ou argocd-vault-plugin-helm
```

Dans ce repo, `bootstrap` et `apps-inventory` déclarent `argocd-vault-plugin` pour que tous les `<path:...>` rencontrés dans leurs sous-dossiers respectifs (et notamment dans les `helm.values` inline des Applications enfants comme `traefik`) soient résolus.
